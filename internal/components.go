...
func (a *AssertionDA) Run(service *service, ctx *ExContext) {
	...
	service.WithImage(name).WithTag("main").WithEntrypoint("assertion-da").WithArgs("listen-addr", `{{Addr "http" 0.0.0.0:5000}}`, "--private-key", a.pk)
}
...
func (o *OpTalos) Run(service *service, ctx *ExContext) {
	service.WithImage("ghcr.io/phylax-systems/op-talos/op-rbuilder").WithTag("main").WithEntrypoint("op-rbuilder").WithArgs("--http.addr", "0.0.0.0", "--http.port", `{{Port "http" 8545}}`)
}
